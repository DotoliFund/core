...
async function main() {
  ...
  await newFund.withdraw(owner.address, WETH9_mainnet, depositAmount);
  ...
  const WETH2 = await ethers.getContractAt("@openzeppelin/contracts/token/ERC20/IERC20.sol:IERC20", WETH9_mainnet);
  console.log("WETH : ", await WETH.balanceOf(owner.address));
