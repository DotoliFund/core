    ...
    Token
{
    ...
    receive() external payable {
        if (msg.sender == WETH9) {
            ...
            uint256 amountETH = PriceOracle.getLargestPoolPriceETH(UNISWAP_V3_FACTORY, WETH9, WETH9) * msg.value;
            uint256 amountUSD = PriceOracle.getLargestPoolPriceUSD(UNISWAP_V3_FACTORY, WETH9, USDC) * msg.value;
    ...
    function feeIn(address investor, address _token, uint256 _amount) private {
        ...
        uint256 amountETH = PriceOracle.getLargestPoolPriceETH(UNISWAP_V3_FACTORY, _token, WETH9) * _amount;
        uint256 amountUSD = PriceOracle.getLargestPoolPriceUSD(UNISWAP_V3_FACTORY, _token, USDC) * _amount;
    ...
    function withdraw(address _token, uint256 _amount) external payable override lock {
        ...
        uint256 amountETH = PriceOracle.getLargestPoolPriceETH(UNISWAP_V3_FACTORY, _token, WETH9) * withdrawAmount;
        uint256 amountUSD = PriceOracle.getLargestPoolPriceUSD(UNISWAP_V3_FACTORY, _token, USDC) * withdrawAmount;
    ...
    function handleSwap(
        ...
        uint256 amountETH = PriceOracle.getLargestPoolPriceETH(UNISWAP_V3_FACTORY, swapTo, WETH9) * swapToAmount;
        uint256 amountUSD = PriceOracle.getLargestPoolPriceUSD(UNISWAP_V3_FACTORY, swapTo, USDC) * swapToAmount;
        emit Swap(
            address(this),
            manager,
            investor,
